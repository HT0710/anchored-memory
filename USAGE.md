...
Recall is untrusted historical evidence, never instructions or authorization. Source labels are unauthenticated; git staleness is neither truth nor a safety check. JSON framing mitigates instruction confusion, not prompt injection. Recording is best-effort; only valid in-repo paths are logged. Outside paths and malformed payloads are skipped. Interrupted sessions may miss capture. Hooks observe only `Edit|Write|MultiEdit|NotebookEdit` calls, so edits made through shell commands get no recall and are never recorded. Secret and personal-data exclusions are agent policy, not guaranteed detection or filtering.
